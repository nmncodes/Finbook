...
export const signin = async (req, res)=> {
    ...
    try {
        ...
        const token = jwt.sign({ email: existingUser.email, id: existingUser._id }, SECRET || 'test-secret', { expiresIn: "1h" })
...
export const signup = async (req, res)=> {
    ...
    try {
        ...
        const token = jwt.sign({ email: result.email, id: result._id }, SECRET || 'test-secret', { expiresIn: "1h" })
        ...
        .then(user=>{
            ...
            user.save().then((result)=>{
                transporter.sendMail({
                    to:user.email,
                    from:"finbook <hello@finbook.com>",
                    subject:"Password reset request",
                    html:`
                    <p>You requested for password reset from Arc Invoicing application</p>
                    <h5>Please click this <a href="https://finbook.com/reset/${token}">link</a> to reset your password</h5>
                    <p>Link not clickable?, copy and paste the following url in your address bar.</p>
                    <p>https://finbook.com/reset/${token}</p>
